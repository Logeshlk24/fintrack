        ...
        {!sidebarCollapsed && (
          <div style={{ marginTop: "auto", padding: "0 0 0.5rem" }}>
            ...
            <div style={{ padding: "0.6rem 1rem", borderTop: "0.5px solid var(--color-border-tertiary)", marginTop: 4 }}>
              <div style={{ fontSize: 12, color: "var(--color-text-secondary)", marginBottom: 4 }}>Signed in as</div>
              <div style={{ fontSize: 13, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{data.user?.name || "User"}</div>
            </div>
          </div>
        )}
        {sidebarCollapsed && (
...
function Onboarding({ step, setStep, data, update, done }) {
  ...
  function handleGoogle() {
    const mockUser = { name: "Demo User", email: "demo@gmail.com", avatar: "DU" };
...
function MoneyPage({ data, update, tab, setTab }) {
  ...
  const [form, setForm] = useState({ type: "expense", amount: "", category: "", note: "", date: today(), bankId: "" });
  const [period, setPeriod] = useState("12M");

  // Account form state
  const [acctForm, setAcctForm] = useState({ name: "", type: "Bank", balance: "", creditLimit: "", dueDate: "" });
  // Adjust balance modal
  const [adjusting, setAdjusting] = useState(null); // account object
  ...
  const income = data.transactions.filter(t => t.type === "income" && filterByPeriod(t.date, period)).reduce((s, t) => s + Number(t.amount), 0);
  const expense = data.transactions.filter(t => t.type === "expense" && filterByPeriod(t.date, period)).reduce((s, t) => s + Number(t.amount), 0);

  const pageTitle = { expenses: "Expenses", income: "Income", accounts: "Accounts", insights: "Money Insights" }[tab];
  ...
  return (
    <div>
      ...
      <TabBar tabs={["expenses", "income", "accounts", "insights"]} active={tab} setActive={setTab} labels={["Expenses", "Income", "Accounts", "Insights"]} />
                  ...
                  return (
                    <div key={acct.id} style={{ background: "var(--color-background-primary)", borderRadius: 14, border: "0.5px solid var(--color-border-tertiary)", padding: "1rem 1.2rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                        ...
                        <button onClick={() => deleteAccount(acct.id)} style={{ background: "none", border: "none", color: "#d44", cursor: "pointer", fontSize: 13 }}>✕</button>
                      </div>
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginTop: 16 }}>
            <Card title={`Add ${tab === "income" ? "Income" : "Expense"}`}>
              ...
              <LabelInput label="Category" placeholder="e.g. Salary, Rent, Food" value={form.category} onChange={v => setForm(p => ({ ...p, category: v }))} />
              ...
              {accounts.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, color: "var(--color-text-secondary)", display: "block", marginBottom: 4 }}>Account</label>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    <button onClick={() => setForm(p => ({ ...p, bankId: "" }))} style={{ padding: "4px 10px", borderRadius: 6, border: "0.5px solid", borderColor: form.bankId === "" ? "#1a6b3c" : "var(--color-border-secondary)", background: form.bankId === "" ? "#e8f5ee" : "transparent", fontSize: 12, cursor: "pointer", color: form.bankId === "" ? "#1a6b3c" : "var(--color-text-secondary)", fontWeight: form.bankId === "" ? 600 : 400 }}>None</button>
                    {accounts.map(b => (
                      <button key={b.id} onClick={() => setForm(p => ({ ...p, bankId: b.id }))} style={{ padding: "4px 10px", borderRadius: 6, border: "0.5px solid", borderColor: form.bankId === b.id ? "#1a6b3c" : "var(--color-border-secondary)", background: form.bankId === b.id ? "#e8f5ee" : "transparent", fontSize: 12, cursor: "pointer", color: form.bankId === b.id ? "#1a6b3c" : "var(--color-text-secondary)", fontWeight: form.bankId === b.id ? 600 : 400 }}>
                        {b.name}{b.type === "Credit Card" ? " 💳" : b.type === "Cash" ? " 💵" : ""}
                      </button>
                    ))}
                  </div>
                </div>
                    ...
                    <tbody>{filtered.slice().reverse().map(t => {
                      ...
                      return (
                        <tr key={t.id} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                          ...
                          <td><button onClick={() => update(p => ({ transactions: p.transactions.filter(x => x.id !== t.id) }))} style={{ background: "none", border: "none", color: "#d44", cursor: "pointer", fontSize: 14 }}>✕</button></td>
                        </tr>
                      );
                    })}</tbody>
                  </table>
            ...
            <Card title="Log New Trade">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                ...
                <div>
                  ...
                  <select value={form.instrument} onChange={e => handleInstrumentChange(e.target.value)} style={{ width: "100%", boxSizing: "border-box" }}>
              ...
              {form.instrument === "Index Options" && (
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                    {indexSubs.map(s => (
                      <button key={s} onClick={() => handleSubInstrumentChange(s)} style={{ padding: "4px 10px", borderRadius: 6, border: "0.5px solid", borderColor: form.subInstrument === s ? "#1a6b3c" : "var(--color-border-secondary)", background: form.subInstrument === s ? "#e8f5ee" : "transparent", fontSize: 12, cursor: "pointer", color: form.subInstrument === s ? "#1a6b3c" : "var(--color-text-secondary)", fontWeight: form.subInstrument === s ? 600 : 400 }}>{s}</button>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {brokerProfiles.map(b => (
                      <button key={b.id} onClick={() => setForm(p => ({ ...p, brokerId: b.id }))} style={{ padding: "4px 12px", borderRadius: 6, border: "0.5px solid", borderColor: form.brokerId === b.id ? "#1a6b3c" : "var(--color-border-secondary)", background: form.brokerId === b.id ? "#e8f5ee" : "transparent", fontSize: 12, cursor: "pointer", color: form.brokerId === b.id ? "#1a6b3c" : "var(--color-text-secondary)", fontWeight: form.brokerId === b.id ? 600 : 400 }}>{b.name}</button>
                    ))}
                    <button onClick={() => setForm(p => ({ ...p, brokerId: "" }))} style={{ padding: "4px 12px", borderRadius: 6, border: "0.5px solid", borderColor: form.brokerId === "" ? "#1a6b3c" : "var(--color-border-secondary)", background: form.brokerId === "" ? "#e8f5ee" : "transparent", fontSize: 12, cursor: "pointer", color: form.brokerId === "" ? "#1a6b3c" : "var(--color-text-secondary)" }}>Default</button>
                  </div>
                </div>
              ) : (
                    ...
                    <tbody>{filtered.slice().reverse().map(t => {
                      ...
                      return (
                        <tr key={t.id} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                          ...
                          <td><button onClick={() => update(p => ({ foTrades: p.foTrades.filter(x => x.id !== t.id) }))} style={{ background: "none", border: "none", color: "#d44", cursor: "pointer", fontSize: 14 }}>✕</button></td>
                        </tr>
                      );
                    })}</tbody>
                  </table>
...
function GoogleBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "10px", border: "0.5px solid var(--color-border-secondary)", borderRadius: 10, background: "var(--color-background-primary)", cursor: "pointer", fontSize: 14, fontWeight: 500, color: "var(--color-text-primary)" }}>
